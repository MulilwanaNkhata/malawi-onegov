...
| SMS/email/push notification gateway | `services/notification-service`: event-driven consumer, SMS adapter (mocked, logging what a gateway like Africa's Talking would send), email via MailHog in dev, and real Web Push (VAPID, not mocked -- no gateway account needed, unlike SMS) to the citizen-portal PWA |
