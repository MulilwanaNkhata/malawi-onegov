...
import { listTradingLicenses, type TradingLicenseApplication } from "../api/onegov";
import { StatusPill } from "../components/StatusPill";

const FILTERS = ["UNDER_REVIEW", "ADDITIONAL_INFO_REQUIRED", "APPROVED", "REJECTED", "ISSUED", ""] as const;

export default function StaffTradingLicenseQueue() {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    setLoading(true);
    listTradingLicenses(status || undefined)
  ...
  return (
    <div className="container">
      <div className="card">
        <h1>
          {t("staffQueue")} — {t("tradingLicenseService")}
        </h1>
          ...
          applications.map((application) => (
            <div className="list-item" key={application.id}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <Link to={`/trading-licenses/${application.id}`}>{t("viewDetails")}</Link>
              </div>
            </div>
          ))
        )}
