...
import {
  getTradingLicense,
  getTradingLicenseCertificateDownload,
  initiatePayment,
  resubmitTradingLicense,
  reviewTradingLicense,
  uploadTradingLicenseDocument,
  type TradingLicenseApplication,
} from "../api/onegov";
import { StatusPill } from "../components/StatusPill";

export default function TradingLicenseDetail() {
  const { id } = useParams<{ id: string }>();
  const { user, isStaff } = useAuth();
  const { t } = useLanguage();

  const [application, setApplication] = useState<TradingLicenseApplication | null>(null);
  ...
  async function reload() {
    if (!id) return;
    const data = await getTradingLicense(id);
    setApplication(data);
  }
  ...
  async function handleUpload(documentType: string, file: File | undefined) {
    ...
    setMessage(null);
    try {
      await uploadTradingLicenseDocument(id, documentType, file);
  ...
  async function handleResubmit() {
    ...
    setBusy(true);
    try {
      await resubmitTradingLicense(id);
      await reload();
    } finally {
      setBusy(false);
    }
  ...
  async function handleReview(action: "REQUEST_MORE_INFO" | "APPROVE" | "REJECT") {
    ...
    setMessage(null);
    try {
      await reviewTradingLicense(id, action);
      await reload();
    } catch {
  ...
  async function handleDownload() {
    if (!id) return;
    const { downloadUrl } = await getTradingLicenseCertificateDownload(id);
  ...
  return (
    <div className="container">
      ...
      <div className="card">
        ...
        <p className="muted">
          {t("businessType")}: {application.businessType} · {t("district")}: {application.district}
        </p>
        <p className="muted">
          {t("tradingAddress")}: {application.tradingAddress}
        </p>
        <p className="muted">{t("ownerFullName")}: {application.ownerFullName}</p>
...
function UploadForm({ onSubmit, busy }: { onSubmit: (documentType: string, file: File | undefined) => void; busy: boolean }) {
  const { t } = useLanguage();
  const [documentType, setDocumentType] = useState("PROOF_OF_ADDRESS");
  ...
  return (
    <div className="field">
      ...
      <select value={documentType} onChange={(e) => setDocumentType(e.target.value)}>
        <option value="PROOF_OF_ADDRESS">Proof of trading address</option>
        <option value="OWNER_NATIONAL_ID">Owner national ID</option>
