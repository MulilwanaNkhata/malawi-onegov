...
export const ENTITY_TYPE = "birth_certificate";
export const WORKFLOW_TEMPLATE_CODE = "birth_certificate";
...
export async function getWorkflowInstance(instanceId: string) {
  const { data } = await axios.get(`${WORKFLOW_SERVICE_URL}/instances/${instanceId}`, {
    headers: serviceHeaders,
    timeout: 5000,
  });
  return data;
...
export async function generateCertificatePdf(input: {
  ...
  const { data } = await axios.post(
    `${DOCUMENT_SERVICE_URL}/files/generate-certificate`,
