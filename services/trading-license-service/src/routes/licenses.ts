...
router.get("/fee-schedule", (_req, res) => {
  res.json({ service: "trading_license", amount: FEE_AMOUNT, currency: FEE_CURRENCY });
});

const createSchema = z.object({
  businessName: z.string().min(2),
  businessType: z.enum(["RETAIL", "RESTAURANT", "SERVICES", "MANUFACTURING", "OTHER"]),
  tradingAddress: z.string().min(2),
...
async function createLicenseApplication(applicantUserId: string, data: z.infer<typeof createSchema>) {
  const referenceNumber = `TL-${new Date().getFullYear()}-${randomUUID().slice(0, 8).toUpperCase()}`;

  const application = await db.tradingLicenseApplication.create({
    data: {
      ...data,
      referenceNumber,
      applicantUserId,
...
async function recordSubmissionAuditAndEvent(
  application: Awaited<ReturnType<typeof createLicenseApplication>>,
  ...
  await recordAuditEvent({
    ...
    actorRole,
    action: "TRADING_LICENSE_APPLICATION_SUBMITTED",
...
router.post("/", requireAuth, requireRole("CITIZEN"), async (req, res) => {
  ...
  const application = await createLicenseApplication(req.user!.sub, parsed.data);
...
router.post("/internal/apply-on-behalf", requireService, async (req, res) => {
  ...
  const application = await createLicenseApplication(applicantUserId, data);
...
router.get("/", requireAuth, async (req, res) => {
  ...
  const applications = await db.tradingLicenseApplication.findMany({
    where: {
...
const ALL_STATUSES = ["SUBMITTED", "UNDER_REVIEW", "ADDITIONAL_INFO_REQUIRED", "APPROVED", "REJECTED", "ISSUED"];
...
router.get("/analytics", requireAuth, requireRole(...STAFF_ROLES), async (_req, res) => {
  const [total, statusCounts, issued] = await Promise.all([
    db.tradingLicenseApplication.count(),
    db.tradingLicenseApplication.groupBy({ by: ["status"], _count: { _all: true } }),
    db.tradingLicenseApplication.findMany({
      where: { status: "ISSUED" },
  ...
  return res.json({ totalApplications: total, byStatus, averageProcessingHours });
});
...
router.get("/:id/certificate", requireAuth, async (req, res) => {
  const application = await db.tradingLicenseApplication.findUnique({ where: { id: req.params.id } });
